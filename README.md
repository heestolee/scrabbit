...
- [3. Next.js를 활용한 프로젝트 구조](#3-nextjs%EB%A5%BC-%ED%99%9C%EC%9A%A9%ED%95%9C-%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B8-%EA%B5%AC%EC%A1%B0)
  ...
  - [3-3. 메인 컨텐츠 기능 조작(CSR)](#3-3-%EB%A9%94%EC%9D%B8-%EC%BB%A8%ED%85%90%EC%B8%A0-%EA%B8%B0%EB%8A%A5-%EC%A1%B0%EC%9E%91csr)
    - [1. url입력 → 로딩 화면 → 스냅샷 렌더링 & 미리보기 컴포넌트 렌더링](#1-url%EC%9E%85%EB%A0%A5-%E2%86%92-%EB%A1%9C%EB%94%A9-%ED%99%94%EB%A9%B4-%E2%86%92-%EC%8A%A4%EB%83%85%EC%83%B7-%EB%A0%8C%EB%8D%94%EB%A7%81--%EB%AF%B8%EB%A6%AC%EB%B3%B4%EA%B8%B0-%EC%BB%B4%ED%8F%AC%EB%84%8C%ED%8A%B8-%EB%A0%8C%EB%8D%94%EB%A7%81)
    - [2. 렌더링된 페이지의 블럭 선택](#2-%EB%A0%8C%EB%8D%94%EB%A7%81%EB%90%9C-%ED%8E%98%EC%9D%B4%EC%A7%80%EC%9D%98-%EB%B8%94%EB%9F%AD-%EC%84%A0%ED%83%9D)
    - [3. 블럭 추가된 상태에서 다른 페이지 렌더링(+블럭 추가)](#3-%EB%B8%94%EB%9F%AD-%EC%B6%94%EA%B0%80%EB%90%9C-%EC%83%81%ED%83%9C%EC%97%90%EC%84%9C-%EB%8B%A4%EB%A5%B8-%ED%8E%98%EC%9D%B4%EC%A7%80-%EB%A0%8C%EB%8D%94%EB%A7%81%EB%B8%94%EB%9F%AD-%EC%B6%94%EA%B0%80)
...
- [4. 구현 세부 사항](#4-%EA%B5%AC%ED%98%84-%EC%84%B8%EB%B6%80-%EC%82%AC%ED%95%AD)
  ...
  - [4-4. 서브도메인과 SSL을 부여하여 SSG로 배포](#4-4-%EC%84%9C%EB%B8%8C%EB%8F%84%EB%A9%94%EC%9D%B8%EA%B3%BC-ssl%EC%9D%84-%EB%B6%80%EC%97%AC%ED%95%98%EC%97%AC-ssg%EB%A1%9C-%EB%B0%B0%ED%8F%AC)
    ...
    - [[2] 배포 페이지 SSL인증서 등록 과정 중 겪게 된 SSL인증서 등록 지연 현상](#2-%EB%B0%B0%ED%8F%AC-%ED%8E%98%EC%9D%B4%EC%A7%80-ssl%EC%9D%B8%EC%A6%9D%EC%84%9C-%EB%93%B1%EB%A1%9D-%EA%B3%BC%EC%A0%95-%EC%A4%91-%EA%B2%AA%EA%B2%8C-%EB%90%9C-ssl%EC%9D%B8%EC%A6%9D%EC%84%9C-%EB%93%B1%EB%A1%9D-%EC%A7%80%EC%97%B0-%ED%98%84%EC%83%81)
...
#### 1. url입력 → 로딩 화면 → 스냅샷 렌더링 & 미리보기 컴포넌트 렌더링
...
#### 3. 블럭 추가된 상태에서 다른 페이지 렌더링(+블럭 추가)
...
| ![alt text](public/image/fetch-page-content.gif)                                   |
| ---------------------------------------------------------------------------------- |
| 입력된 url의 스냅샷을 생성하여 렌더링하고, 부분적으로 선택하여 추출할 수 있습니다. |
...
동일한 로직으로 렌더링하는데 왜 CSS속성이 부여되지 않는 경우도 있는지 확인해보니, 원인은 웹 페이지의 CSS 적용방식에 따른 차이 때문이었습니다.
...
- 각 블럭은 고유 식별자인 `data-block-id`를 통해 관리됩니다.
...
배포할 요소를 선택하면 미리보기 컴포넌트가 같이 업데이트 되도록 선택 상태를 `useState`에 배열로 저장하고, 그 상태를 부모 컴포넌트로 끌어올려 미리보기 컴포넌트에 공유되도록 설계하였습니다.

이를 통해 사용자는 선택한 요소가 실제로 어떻게 배포될지를 직관적으로 확인할 수 있게 하고, 추가로 빈번한 상태 업데이트로 인한 성능 저하를 줄이기 위해 `useRef`를 사용하여 불필요한 리렌더링을 방지했습니다.
...
### [2] 배포 페이지 SSL인증서 등록 과정 중 겪게 된 SSL인증서 등록 지연 현상
